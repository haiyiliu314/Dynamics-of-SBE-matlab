...
%     f(n+1) = runge_kuttaf(f(n), p(n), dt, n);
% end
topp(i) = abs(abs(p(tgrid1+1)) - sqrt(0.1*((1-exp(-gamma*t_end1)) / (gamma*1e-12))^2)) / sqrt(abs(0.1*((1-exp(-gamma*t_end1)) / (gamma*1e-12))^2));
end
plot((1:(tgrid1+1))*dt, abs(p).^2);
hold on
plot((1:(tgrid1+1))*dt, 0.1*((1-exp(-gamma*((0:tgrid1))*dt)) / (gamma*1e-12)).^2);
figure
loglog(2.^(0:(j-1)),topp)
hold on
